"""Minimal BASE Segmentation: Render Retained Movement Clips and Timelines.

Produces retained movement clips with 150 ms pre-roll and 200 ms post-roll
for Recording A and Recording B under the minimal generic BASE / NORMAL model.
"""
...
SRC_A = ROOT / "output/task5/base_segmentation/recording_a"
SRC_B = ROOT / "output/task5/base_segmentation/recording_b"

DEST = ROOT / "docs/validation/task5/base-clips"
...
def generate_timeline_plot(frames: list[dict], segments: list[dict], title: str, out_path: Path) -> None:
    ...
    ax1.set_title(f"{title} - Minimal Generic BASE Kinematic Segmentation")
...
def render_movement_clip(
    ...
    for frame_idx in range(f_start, f_end + 1):
        ...
        overlay = frame.copy()
        cv2.rectangle(overlay, (10, 10), (width - 10, 105), (0, 0, 0), -1)
...
def main():
    print("Generating Minimal BASE retained video clips and timeline plots...")
    ...
    generate_timeline_plot(trace_a["frames"], res_a["segments"], "Recording A (Minimal BASE)", DEST_A / "recording-a-timeline.png")
    ...
    generate_timeline_plot(trace_b["frames"], res_b["segments"], "Recording B (Minimal BASE)", DEST_B / "recording-b-timeline.png")
    ...
    print("Minimal BASE clip rendering complete!")
