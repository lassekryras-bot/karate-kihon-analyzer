...
class FindYourWeaponVerifierTest {
    ...
    @Test fun thumbOnTopRequiresReliableThumbAcrossAndMirrors() {
        ...
        assertEquals(InstantVerificationStatus.INSUFFICIENT_DATA, verify(HandLessonStep.THUMB_ON_TOP, hand(curl = 70f, missing = setOf(HandLandmarkId.THUMB_TIP))).status)
    ...
    @Test fun frontTwoKnucklesHighlightsAndHandlesLimitedOrientation() {
        ...
        assertEquals(InstantVerificationStatus.INSUFFICIENT_DATA, verify(HandLessonStep.FRONT_TWO_KNUCKLES, hand(curl = 70f, missing = setOf(HandLandmarkId.INDEX_MCP))).status)
    ...
    private fun hand(handedness: Handedness = Handedness.RIGHT, curl: Float = 175f, thumbCrossing: Boolean = false, missing: Set<HandLandmarkId> = emptySet(), source: LandmarkSource = LandmarkSource.OBSERVED, scale: Float = 1f, offset: Point3 = Point3(0f,0f,0f), perFinger: List<Float>? = null): TrackedHandFrame {
        ...
        finger(map, HandLandmarkId.INDEX_MCP,HandLandmarkId.INDEX_PIP,HandLandmarkId.INDEX_DIP,HandLandmarkId.INDEX_TIP,-.6f,1f,curls[0],scale,offset,mirror)
        finger(map, HandLandmarkId.MIDDLE_MCP,HandLandmarkId.MIDDLE_PIP,HandLandmarkId.MIDDLE_DIP,HandLandmarkId.MIDDLE_TIP,-.2f,1f,curls[1],scale,offset,mirror)
        finger(map, HandLandmarkId.RING_MCP,HandLandmarkId.RING_PIP,HandLandmarkId.RING_DIP,HandLandmarkId.RING_TIP,.2f,1f,curls[2],scale,offset,mirror)
        finger(map, HandLandmarkId.LITTLE_MCP,HandLandmarkId.LITTLE_PIP,HandLandmarkId.LITTLE_DIP,HandLandmarkId.LITTLE_TIP,.6f,1f,curls[3],scale,offset,mirror)
    ...
    private fun finger(m: MutableMap<HandLandmarkId, Point3>, a:HandLandmarkId,b:HandLandmarkId,c:HandLandmarkId,d:HandLandmarkId,x:Float,y:Float,ang:Float,scale:Float,off:Point3,mir:Float){ val p=t(x,y,scale,off,mir); val q=p+v(.7f,90.0,scale,mir); val r=q+v(.6f,270.0-ang,scale,mir); val s=r+v(.5f,450.0-2*ang,scale,mir); m[a]=p;m[b]=q;m[c]=r;m[d]=s }
